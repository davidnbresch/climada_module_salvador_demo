function hazard = climada_asci2hazard_sansal(asci_file)
% climada_asci2hazard
% MODULE:
%   climada core
% NAME:
...
% Jacob Anz,j.anz@gmx.net,20150721, specific shift for San Salvador
...
asci_dir = [module_data_dir filesep 'hazards' filesep 'output' filesep];
...
if isempty(asci_file) % local GUI
    ...
    if ~exist(asci_dir, 'dir')      
        asci_dir=[climada_global.data_dir filesep 'hazards' filesep];
    end
    % add asci-extension
    asci_dir= [asci_dir '*.asc'];
    % ask for asci-filename
    [filename, pathname] = uigetfile(asci_dir, 'Select asci (raster hazard event from external model):');
...
row_count = 6;                                                                                                      %specific for san salvador flood data
%% read asci-file
 delimiter = ' ';
%delimiter = '\t';
event_grid = flipud(dlmread(asci_file,delimiter,row_count,0));

event_grid(:,length(event_grid))=[];                                                                                %specific for san salvador flood data
...
hazard.peril_ID         = 'FL';
hazard.comment          = [name_only ' - ' 'modelled by W+B'];
...
hazard_filename = [pathstr filesep 'hazard_' name_only '.mat'];
